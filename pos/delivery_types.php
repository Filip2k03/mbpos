<?php
// pos/delivery_types.php - CRUD management for delivery types (V3 Premium UI).
...
<div class="relative min-h-[85vh] bg-gray-50/30 p-4 sm:p-8 overflow-hidden font-sans">
    ...
    <div class="absolute top-[10%] left-[-10%] w-[500px] h-[500px] bg-cyan-400/10 rounded-full blur-[120px] pointer-events-none"></div>
    <div class="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-blue-400/10 rounded-full blur-[120px] pointer-events-none"></div>

    <div class="max-w-7xl mx-auto relative z-10">
        
        <!-- Header -->
        <div class="flex items-center gap-4 mb-8">
            <div class="w-14 h-14 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 text-white transform -rotate-3 hover:rotate-0 transition-transform duration-300">
            ...
            <div>
                <h1 class="text-3xl font-extrabold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent tracking-tight">Delivery Service Types</h1>
                <p class="text-sm font-medium text-gray-500">Configure logistics service levels and shipping speeds</p>
            </div>
        </div>
                ...
                <div class="bg-white/80 backdrop-blur-2xl p-6 sm:p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 sticky top-28">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                            <svg class="w-5 h-5 <?= $edit_type ? 'text-amber-500' : 'text-blue-500' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <?php if($edit_type): ?>
                            ...
                            <?= $edit_type ? 'Edit Service' : 'Add Service' ?>
                        </h2>
                        <?php if($edit_type): ?>
                            <a href="index.php?page=delivery_types" class="text-xs font-bold text-gray-400 hover:text-red-500 transition-colors uppercase tracking-wider">Cancel</a>
                        <?php endif; ?>
                    </div>
                    
                    <!-- Enforce UTF-8 form submission for Myanmar language -->
                    <form action="index.php?page=delivery_types" method="POST" accept-charset="UTF-8" class="space-y-5">
                        <input type="hidden" name="type_id" value="<?= $edit_type['id'] ?? '' ?>">
                        
                        <div class="space-y-1.5 group">
                            <label for="name" class="block text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Service Name</label>
                            <input type="text" id="name" name="name" class="w-full rounded-2xl border-gray-200 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all py-3 px-4 font-medium text-gray-800 shadow-sm placeholder-gray-300" placeholder="e.g. Express / အမြန်" value="<?= htmlspecialchars($edit_type['name'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required>
                        ...
                        <div class="pt-4">
                            ...
                            <?php else: ?>
                                <button type="submit" class="w-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white py-3.5 px-4 rounded-2xl font-bold text-md hover:from-cyan-600 hover:to-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-500/30 shadow-[0_8px_20px_rgb(59,130,246,0.3)] transition-all transform hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center gap-2">
            ...
            <div class="lg:col-span-8">
                <div class="bg-white/70 backdrop-blur-2xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 overflow-hidden h-full flex flex-col">
                    ...
                    <div class="p-6 sm:p-8 border-b border-gray-100 flex items-center justify-between bg-white/50">
                        ...
                        <span class="bg-blue-50 text-blue-600 py-1 px-3 rounded-full text-xs font-bold border border-blue-100 shadow-sm">
                            <?= count($delivery_types) ?> Services
                        </span>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse whitespace-nowrap">
                            <thead>
                                <tr>
                                    ...
                                    <th class="py-4 px-6 text-xs font-extrabold text-gray-400 uppercase tracking-widest border-b border-gray-100">Service Name</th>
                            ...
                            <tbody class="divide-y divide-gray-100/60">
                                <?php if (empty($delivery_types)): ?>
                                    <tr>
                                        <td colspan="3" class="py-12 text-center text-gray-400 font-medium">No delivery services configured.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($delivery_types as $type): ?>
                                        <tr class="hover:bg-white/90 transition-colors duration-200 group">
                                            ...
                                            <td class="py-4 px-6">
                                                ...
                                                <span class="font-bold text-gray-800"><?= htmlspecialchars($type['name'], ENT_QUOTES, 'UTF-8') ?></span>
                                            </td>
                                            <td class="py-4 px-6 text-right space-x-2">
                                                <a href="index.php?page=delivery_types&action=edit&id=<?= $type['id'] ?>" class="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-gray-50 text-gray-500 hover:text-amber-500 hover:bg-amber-50 hover:shadow-sm border border-transparent hover:border-amber-100 transition-all" title="Edit">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                </a>
                                                <a href="index.php?page=delivery_types&action=delete&id=<?= $type['id'] ?>" onclick="return confirm('Are you sure you want to delete the delivery service <?= htmlspecialchars($type['name'], ENT_QUOTES, 'UTF-8') ?>? This action cannot be undone.');" class="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-gray-50 text-gray-500 hover:text-red-500 hover:bg-red-50 hover:shadow-sm border border-transparent hover:border-red-100 transition-all" title="Delete">
